...
! function($) {
    ...
    $.fn.vectorMap = function(options) {
        ...
        if ("addMap" === options) WorldMap.maps[arguments[1]] = arguments[2];
        else if ("set" === options && apiParams[arguments[1]]) map["set" + arguments[1].charAt(0).toUpperCase() + arguments[1].substr(1)].apply(map, Array.prototype.slice.call(arguments, 2));
        else {
            if ("string" == typeof options && "function" == typeof map[options]) return map[options].apply(map, Array.prototype.slice.call(arguments, 1));
            $.extend(defaultParams, options), defaultParams.container = this, this.css({
                position: "relative",
                overflow: "hidden"
            }), map = new WorldMap(defaultParams), this.data("mapObject", map);
    ...
    VectorCanvas.prototype = {
        ...
        setSize: function(width, height) {
            if ("svg" == this.mode) this.canvas.setAttribute("width", width), this.canvas.setAttribute("height", height);
                ...
                for (var pathes = this.rootGroup.getElementsByTagName("shape"), i = 0, l = pathes.length; l > i; i++) pathes[i].coordsize = width + " " + height, pathes[i].style.width = width + "px", pathes[i].style.height = height + "px";
        ...
        createPath: function(config) {
            ...
            if ("svg" == this.mode) node = this.createSvgNode("path"), node.setAttribute("d", config.path), null !== this.params.borderColor && node.setAttribute("stroke", this.params.borderColor), this.params.borderWidth > 0 && (node.setAttribute("stroke-width", this.params.borderWidth), node.setAttribute("stroke-linecap", "round"), node.setAttribute("stroke-linejoin", "round")), this.params.borderOpacity > 0 && node.setAttribute("stroke-opacity", this.params.borderOpacity), node.setFill = function(color) {
                ...
                node = this.createVmlNode("shape"), node.coordorigin = "0 0", node.coordsize = this.width + " " + this.height, node.style.width = this.width + "px", node.style.height = this.height + "px", node.fillcolor = WorldMap.defaultFillColor, node.stroked = !1, node.path = VectorCanvas.pathSvgToVml(config.path);
    ...
    var WorldMap = function(params) {
        ...
        this.selectedRegions = [], this.multiSelectRegion = params.multiSelectRegion, this.container = params.container, this.defaultWidth = mapData.width, this.defaultHeight = mapData.height, this.color = params.color, this.selectedColor = params.selectedColor, this.hoverColor = params.hoverColor, this.hoverOpacity = params.hoverOpacity, this.setBackgroundColor(params.backgroundColor), this.width = params.container.width(), this.height = params.container.height(), this.resize(), jQuery(window).resize(function() {
            map.width = params.container.width(), map.height = params.container.height(), map.resize(), map.canvas.setSize(map.width, map.height), map.applyTransform()
        }), this.canvas = new VectorCanvas(this.width, this.height, params), params.container.append(this.canvas.canvas), this.makeDraggable(), this.rootGroup = this.canvas.createGroup(!0), this.index = WorldMap.mapIndex, this.label = jQuery("<div/>").addClass("jqvmap-label").appendTo(jQuery("body")).hide(), params.enableZoom && (jQuery("<div/>").addClass("jqvmap-zoomin").text("+").appendTo(params.container), jQuery("<div/>").addClass("jqvmap-zoomout").html("&#x2212;").appendTo(params.container)), map.countries = [];
        for (var key in mapData.pathes) {
            var path = this.canvas.createPath({
                path: mapData.pathes[key].path
            });
            path.setFill(this.color), path.id = map.getCountryId(key), map.countries[key] = path, "svg" == this.canvas.mode ? path.setAttribute("class", "jvectormap-region") : jQuery(path).addClass("jvectormap-region"), jQuery(this.rootGroup).append(path)
        }
        if (jQuery(params.container).delegate("svg" == this.canvas.mode ? "path" : "shape", "mouseover mouseout", function(e) {
                ...
                "mouseover" == e.type ? (jQuery(params.container).trigger(regionMouseOverEvent, [code, mapData.pathes[code].name]), regionMouseOverEvent.isDefaultPrevented() || map.highlight(code, path), params.showTooltip && (map.label.text(mapData.pathes[code].name), jQuery(params.container).trigger(labelShowEvent, [map.label, code]), labelShowEvent.isDefaultPrevented() || (map.label.show(), map.labelWidth = map.label.width(), map.labelHeight = map.label.height()))) : (map.unhighlight(code, path), map.label.hide(), jQuery(params.container).trigger("regionMouseOut.jqvmap", [code, mapData.pathes[code].name]))
            }), jQuery(params.container).delegate("svg" == this.canvas.mode ? "path" : "shape", "click", function(e) {
                if (!params.multiSelectRegion)
                    for (var key in mapData.pathes) map.countries[key].currentFillColor = map.countries[key].getOriginalFill(), map.countries[key].setFill(map.countries[key].getOriginalFill());
                var path = e.target,
                    code = e.target.id.split("_").pop();
                jQuery(params.container).trigger("regionClick.jqvmap", [code, mapData.pathes[code].name])//, regionClickEvent.isDefaultPrevented() || (-1 !== map.selectedRegions.indexOf(code) ? map.deselect(code, path) : map.select(code, path))
        ...
        this.bindZoomButtons(), params.pins && (this.pinHandlers = !1, this.placePins(params.pins, params.pinMode)), WorldMap.mapIndex++
    };
    WorldMap.prototype = {
        transX: 0,
        transY: 0,
        ...
        setValues: function(values) {
            ...
            for (var cc in values) val = parseFloat(values[cc]), val > max && (max = values[cc]), val && min > val && (min = val);
            this.colorScale.setMin(min), this.colorScale.setMax(max);
            var colors = {};
            for (cc in values) val = parseFloat(values[cc]), val ? colors[cc] = this.colorScale.getColor(val) : colors[cc] = this.color;
        ...
        highlight: function(cc, path) {
            path = path || $("#" + this.getCountryId(cc))[0], this.hoverOpacity ? path.setOpacity(this.hoverOpacity) : this.hoverColor && (path.currentFillColor = path.getFill() + "", path.setFill(this.hoverColor))
        },
        unhighlight: function(cc, path) {
            path = path || $("#" + this.getCountryId(cc))[0], path.setOpacity(1), path.currentFillColor && path.setFill(path.currentFillColor)
        },
        select: function(cc, path) {
            path = path || $("#" + this.getCountryId(cc))[0], this.selectedRegions.indexOf(cc) < 0 && (this.multiSelectRegion ? this.selectedRegions.push(cc) : this.selectedRegions = [cc], $(this.container).trigger("regionSelect.jqvmap", [cc]), this.selectedColor && (path.currentFillColor = this.selectedColor, path.setFill(this.selectedColor)))
        },
        deselect: function(cc, path) {
            path = path || $("#" + this.getCountryId(cc))[0], this.selectedRegions.indexOf(cc) >= 0 && (this.selectedRegions.splice(this.selectedRegions.indexOf(cc), 1), $(this.container).trigger("regionDeselect.jqvmap", [cc]), path.currentFillColor = path.getOriginalFill(), path.setFill(path.getOriginalFill()))
        },
        ...
        reset: function() {
            ...
            for (var key in this.countries) this.countries[key].setFill(WorldMap.defaultColor);
        ...
        makeDraggable: function() {
            var oldPageX, oldPageY, mouseDown = !1,
                self = this;
            self.isMoving = !1, self.isMovingTimeout = !1, this.container.mousemove(function(e) {
                if (mouseDown) {
                    self.transX, self.transY;
                    self.transX -= (oldPageX - e.pageX) / self.scale, self.transY -= (oldPageY - e.pageY) / self.scale, self.applyTransform(), oldPageX = e.pageX, oldPageY = e.pageY, self.isMoving = !0, self.isMovingTimeout && clearTimeout(self.isMovingTimeout), self.container.trigger("drag")
                }
                return !1
            }).mousedown(function(e) {
                return mouseDown = !0, oldPageX = e.pageX, oldPageY = e.pageY, !1
            }).mouseup(function() {
                return mouseDown = !1, self.isMovingTimeout = setTimeout(function() {
                    self.isMoving = !1
                }, 100), !1
            })
        },
        ...
        zoomIn: function() {
            ...
            if (map.zoomCurStep < map.zoomMaxStep) {
                map.transX, map.transY, map.scale;
                map.transX -= (map.width / map.scale - map.width / (map.scale * map.zoomStep)) / 2, map.transY -= (map.height / map.scale - map.height / (map.scale * map.zoomStep)) / 2, map.setScale(map.scale * map.zoomStep), map.zoomCurStep++, jQuery("#zoomSlider").css("top", parseInt(jQuery("#zoomSlider").css("top"), 10) - sliderDelta), map.container.trigger("zoomIn")
            }
        },
        zoomOut: function() {
            var map = this,
            ...
            if (map.zoomCurStep > 1) {
                map.transX, map.transY, map.scale;
                map.transX += (map.width / (map.scale / map.zoomStep) - map.width / map.scale) / 2, map.transY += (map.height / (map.scale / map.zoomStep) - map.height / map.scale) / 2, map.setScale(map.scale / map.zoomStep), map.zoomCurStep--, jQuery("#zoomSlider").css("top", parseInt(jQuery("#zoomSlider").css("top"), 10) + sliderDelta), map.container.trigger("zoomOut")
            }
        },
        ...
        placePins: function(pins, pinMode) {
            ...
            if ((!pinMode || "content" != pinMode && "id" != pinMode) && (pinMode = "content"), "content" == pinMode ? jQuery.each(pins, function(index, pin) {
                    if (0 != jQuery("#" + map.getCountryId(index)).length) {
                        ...
                        jQuery("#" + pinIndex).length > 0 && jQuery("#" + pinIndex).remove(), map.container.append('<div id="' + pinIndex + '" for="' + index + '" class="jqvmap_pin" style="position:absolute">' + pin + "</div>")
                    }
                }) : jQuery.each(pins, function(index, pin) {
                    if (0 != jQuery("#" + map.getCountryId(index)).length) {
                        var pinIndex = map.getPinId(index);
                        jQuery("#" + pinIndex).length > 0 && jQuery("#" + pinIndex).remove(), map.container.append('<div id="' + pinIndex + '" for="' + index + '" class="jqvmap_pin" style="position:absolute"></div>'), jQuery("#" + pinIndex).append(jQuery("#" + pin))
                    }
    ...
    }, WorldMap.xlink = "http://www.w3.org/1999/xlink", WorldMap.mapIndex = 1, WorldMap.maps = {};
